...
global.ResizeObserver = class ResizeObserver {
  ...
  observe(target: Element) {
    ...
    // Immediately call callback with mock size for canvas containers
    if (target.tagName === 'DIV' && target.firstChild?.tagName === 'CANVAS') {
      queueMicrotask(() => this.callback([{ contentRect: { width: 800, height: 600, top: 0, left: 0, right: 800, bottom: 600, x: 0, y: 0, toJSON: () => {} } }], this));
    }
  }
